...
class OfflineTtsVitsImpl : public OfflineTtsImpl {
 ...
 private:
  ...
  GeneratedAudio Process(const std::vector<std::vector<int64_t>> &tokens,
    ...
    /*刘家骏*/
    // 打印张量数据
    const int64_t* data = x_tensor.GetTensorMutableData<int64_t>();
    size_t data_size = x_tensor.GetTensorTypeAndShapeInfo().GetElementCount();
